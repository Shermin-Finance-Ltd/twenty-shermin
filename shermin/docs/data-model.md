...
| Contract document | `contractDoc` | Link | No | URL to the signed contract in document storage. | Twenty v2.1.0 has no first-class file field on custom objects. **Recommended pattern:** attach contract / compliance / KYC documents as **`Note` records related to the retailer** — Note is a standard Twenty object with native attachment support. Use this `contractDoc` Link only for externally-hosted docs (DocuSign envelope URL). v0 image lifts the per-file cap to 100 MB (see [v0-deploy-log.md](v0-deploy-log.md#100mb-upload-cap)). See open question. |
...
- **`note`** — attached to `retailer` for free-form notes (call notes, ad-hoc context) AND for **document storage** (signed contracts, compliance evidence, KYC docs). Note has native attachment support whereas custom objects don't, so we use Note as the carrier for any retailer-related file in v0/v1. The 100 MB per-file cap covers everything bar full HD video.
...
1. **Contract document storage — settled for v0.** Twenty v2.1.0 doesn't expose a first-class file field on custom objects. We use `Note` records (native attachment support) related to the retailer as the document carrier. `contractDoc` Link field on `retailer` stays for externally-hosted docs (DocuSign envelope URL). v0 image patched to 100 MB per file. Re-open this question only if Twenty ships first-class file fields on custom objects in a later release — at that point we'd consider migrating from Notes to direct attachment.
